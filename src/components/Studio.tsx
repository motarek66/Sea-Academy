...
export default function Studio() {
  ...
  async function openPicker(item: VocabularyItem) {
    setImagePicker(item); setPickerResults([]); setPickerLoading(true);
    try {
      const r = await fetch('/api/images/search', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ word: item.word, sense: item.searchHint }) });
      const data = await r.json(); setPickerResults(data.results || []);
  ...
  return (
    <div className="app" dir={rtl ? 'rtl' : 'ltr'}>
      ...
      {imagePicker && <Modal wide onClose={() => setImagePicker(null)}>
        ...
        <div className="picker-tools"><span>Openverse · Unsplash · Pixabay · Google</span><label className="upload-btn"><Upload size={15}/> Upload image<input type="file" accept="image/*" onChange={(e) => handleUpload(e.target.files?.[0])}/></label></div>
